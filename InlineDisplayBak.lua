...
local mod = Skada:NewModule("InlineDisplay", "SpecializedLibBars-1.0")
...
--mod.name = L["Inline display"]
mod.name = "Inline display"
Skada.displays["inline"] = mod


function customCreate(window)
    --
    --custom Inline Display code
    --

    local bargroup = window.bargroup

    inlinebar.frame = CreateFrame("Frame", "TestFrame", UIParent)
    inlinebar.background = inlinebar.frame:CreateTexture("TestFrameBackground", "BACKGROUND")
    inlinebar.ft = inlinebar.frame:CreateFontString("TestFrameFontstringTitle", 6)
    inlinebar.fs = inlinebar.frame:CreateFontString("TestFrameFontstringText", 6)
    inlinebar.b1 = CreateFrame("Button", "SkadaInlineButton1", UIParent)
    inlinebar.b2 = CreateFrame("Button", "SkadaInlineButton2", UIParent)

    local title = inlinebar.title or "Skada"
    local text = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.";

    inlinebar.background:SetTexture(1, 1, 1, 1.00)
    inlinebar.background:SetAllPoints()
    inlinebar.frame:SetFrameLevel(5)
    inlinebar.frame:SetHeight(23)
    inlinebar.frame:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", -1, -1)
    inlinebar.frame:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", 1, -1)

    inlinebar.ft:SetTextColor(0,0,0,1.00)
    inlinebar.ft:SetFont([[Interface\AddOns\SkadaInline\media\fonts\PT_Sans_Narrow.ttf]], 10, nil)
    inlinebar.ft:SetText(title)
    inlinebar.ft:SetWordWrap(false)
    inlinebar.ft:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 20, -1)
    inlinebar.ft:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMLEFT", 200, -1)
    inlinebar.ft:SetHeight(23)

    inlinebar.fs:SetTextColor(0,0,0,1.00)
    inlinebar.fs:SetFont([[Interface\AddOns\SkadaInline\media\fonts\PT_Sans_Narrow.ttf]], 10, nil)
    inlinebar.fs:SetText(text)
    inlinebar.fs:SetWordWrap(false)
    inlinebar.fs:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 200, -1)
    inlinebar.fs:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", 1, -1)
    inlinebar.fs:SetHeight(23)

    inlinebar.b1:SetFrameLevel(7)
    inlinebar.b1:ClearAllPoints()
    inlinebar.b1:SetHeight(12)
    inlinebar.b1:SetWidth(12)
    inlinebar.b1:SetNormalTexture("Interface\\Buttons\\UI-GuildButton-PublicNote-Up")
    inlinebar.b1:SetHighlightTexture("Interface\\Buttons\\UI-GuildButton-PublicNote-Up")
    inlinebar.b1:SetAlpha(0.5)
    inlinebar.b1:SetPoint("CENTER")
    inlinebar.b1:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 1, 5)
    inlinebar.b1:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMLEFT", 13, 5)
    inlinebar.b1:SetScript("OnClick",
        function()
            print("Mode")
            Skada:SegmentMenu(inlinebar.frame)
            --Skada:SegmentMenu(bargroup.window)
        end)
    inlinebar.b1:SetScript("OnEnter",
        function(this)
            GameTooltip_SetDefaultAnchor(GameTooltip, this)
            GameTooltip:SetText("Mode")
            GameTooltip:AddLine("Jump to a specific mode.", 1, 1, 1, true)
            GameTooltip:Show()
        end)
    inlinebar.b1:SetScript("OnLeave", function() GameTooltip:Hide() end)

    inlinebar.b2:SetFrameLevel(7)
    inlinebar.b2:ClearAllPoints()
    inlinebar.b2:SetHeight(12)
    inlinebar.b2:SetWidth(12)
    inlinebar.b2:SetNormalTexture("Interface\\Buttons\\UI-GuildButton-PublicNote-Up")
    inlinebar.b2:SetHighlightTexture("Interface\\Buttons\\UI-GuildButton-PublicNote-Up")
    inlinebar.b2:SetAlpha(0.5)
    inlinebar.b2:SetPoint("CENTER")
    inlinebar.b2:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", 13, 5)
    inlinebar.b2:SetPoint("BOTTOMRIGHT", UIParent, "BOTTOMLEFT", 25, 5)
    inlinebar.b2:SetScript("OnClick", function()
        print("Segment")
        Skada:ModeMenu(inlinebar.frame)
        --Skada:ModeMenu(bargroup.window)
    end)
    inlinebar.b2:SetScript("OnEnter",
        function(this)
            GameTooltip_SetDefaultAnchor(GameTooltip, this)
            GameTooltip:SetText("Segment")
            GameTooltip:AddLine("Jump to a specific segment.", 1, 1, 1, true)
            GameTooltip:Show()
        end)
    inlinebar.b2:SetScript("OnLeave", function() GameTooltip:Hide() end)


    --[[inlinebar.bu:AddButton(L["Configure"],
...
function mod:CustomCreate2(window)
    print("mod:CustomCreate2()")
    print(window.db.name)
...
function mod:Update(win)
    ...
    local title = win.metadata.title
    local inline = {}
    datasettable = win.dataset
    datasetsize = #datasettable

    local hasdataset = false
    ...
    --title
    inlinebar.title = win.metadata.title



--    print(exists("valuetext", datasettable))
--    print(exists("value", win.dataset))
--    print(exists("Exac", win.dataset))
--    print(exists("role", win.dataset))

--    for index = 1, datasetsize do
--        local context = win.dataset[index].label
--        context = context .."  "
--        if(valid('valuetext', win.dataset[index])) then
--            context = context ..win.dataset[index].valuetext
--            print("valid")
--        else
--            print("invalid");
--        end
--        context = win.dataset[index].label
--        print(context)
--        index = index + 1
--    end
